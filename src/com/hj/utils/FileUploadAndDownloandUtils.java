...
public String handlerFormUpload(String name,List<MultipartFile> uploadfile,
		...
		for (MultipartFile file : uploadfile) {
			...
			String dirpath="F:\\Users\\17466\\Desktop\\myProject-master(1) (2)\\文件"; //重点标记
			File filepath=new File(dirpath);
			if(!filepath.exists()) {
				filepath.mkdirs();
			}
			//重命名上传的文件名称
			String newFilename = name+"_"+idcard+"_"+ayear+"宽带账号密码.txt";
			//ʹ使用multipartfile接口的方法完成文件上传到指定位置
			try {
	...
	public ResponseEntity<byte[]> dowbLoad(String filename,HttpServletRequest request) throws IOException {
		...
		String path="F:\\Users\\17466\\Desktop\\myProject-master(1) (2)\\文件";
		//创建文件对象
